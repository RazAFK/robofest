...
int railLength; // длина рейки
int angle0; // нулевой угол
...
void executeCommand(String cmd, int arg1, int arg2) {
  if (cmd == "getPlate") { // проверка платы
    ...
    int manipulatorPos = (int)round(sqrt(pow(railLength * cos(angle0) - arg1, 2) + pow(railLength * sin(angle0) - arg2, 2))); // позиция манипулятора на горизонтальной рейке
    int railAngle = (int)round(acos((railLength * cos(angle0) - arg1)/manipulatorPos)); // новый угол рейки

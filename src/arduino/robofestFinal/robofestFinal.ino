...
public:
    EncoderMotor(int I2CAddress,
                 ...
                 float reducer,
                 float wheelRadius,
                 bool defaultDirection) : motor(iarduino_I2C_Motor(I2CAddress)),
...
public:
    WheelMotor(int I2CAddress,
               ...
               float reducer,
               float wheelRadius,
               bool defaultDirection) : EncoderMotor(I2CAddress,
                                                     magnetsCount,
                                                     reducer,
                                                     wheelRadius,
                                                     defaultDirection) {}
...
public:
    Manipulator(Servo &grabServo,
                ...
                ServoSmooth &railRotationServo,
                GStepper<STEPPER2WIRE> &horizontalRailMotor,
                VerticalRailMotor &verticalRailMotor) : grabServo(grabServo),
    ...
    void moveVerticalRail(float speed, float distance);
...
public:
    WheelBase(WheelMotor &forwardRight,
              WheelMotor &forwardLeft,
              WheelMotor &backwardRight,
              WheelMotor &backwardLeft) : forwardRight(forwardRight),
...
const String MessageHandler::prefixes[] = {
    "arm",
    "whe",
    "data"}; // егор дибил нормальные не придумал и переделывать не хочет
...
void Manipulator::moveVerticalRail(float speed, float distance)
{
    verticalRailMotor.move(speed, distance);
}
...
void MessageHandler::executeCommand(String command, float *arguments)
{
    ...
    else if (command == "getGrabServoDegrees")
    {
        ...
        digitalWrite(PIN_STEPPER_DIR, (int)arguments[0] == 0 ? LOW : HIGH);
    }
}